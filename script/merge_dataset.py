...
def merge_dataset():
    ...
    pickle_folder_list = os.listdir(my_config.MERGE_PATH)

    print(pickle_folder_list)

    out_lmdb_dir_train = my_config.MERGE_PATH + '/train'
    out_lmdb_dir_val = my_config.MERGE_PATH + '/val'
    out_lmdb_dir_test = my_config.MERGE_PATH + '/test'
    ...
    for dir in pickle_folder_list:
        pickle_file = my_config.MERGE_PATH + '/' + dir + '/trainted_expressive_dataset_train.pickle'
        with open(pickle_file, 'rb') as file:
            temp_train = pickle.load(file)
        dataset_train.extend(temp_train)

    for dir in pickle_folder_list:
        pickle_file = my_config.MERGE_PATH + '/' + dir + '/trainted_expressive_dataset_val.pickle'
        with open(pickle_file, 'rb') as file:
            temp_val = pickle.load(file)
        dataset_val.extend(temp_val)

    for dir in pickle_folder_list:
        pickle_file = my_config.MERGE_PATH + '/' + dir + '/trainted_expressive_dataset_test.pickle'
        with open(pickle_file, 'rb') as file:
            temp_test = pickle.load(file)
        dataset_test.extend(temp_test)

    print('writing to pickle...')
    with open(my_config.MERGE_PATH + '/' + 'trainted_expressive_dataset_train.pickle', 'wb') as f:
        pickle.dump(dataset_train, f)
    with open(my_config.MERGE_PATH + '/' + 'trainted_expressive_dataset_val.pickle', 'wb') as f:
        pickle.dump(dataset_val, f)
    with open(my_config.MERGE_PATH + '/' + 'trainted_expressive_dataset_test.pickle', 'wb') as f:
