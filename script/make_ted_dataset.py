        ...
        for ia, clip in enumerate(clip_data):
            ...
            if clip_word_list:
                ...
                for frame in clip_pose_3d[:-1]:
                    if frame:
                        joints_full = frame['joints']
                        up_joints = np.vstack((joints_full[9], joints_full[12], joints_full[16:22], joints_full[25:60])).astype('float32')
                        clip_skeleton_3d.append(up_joints)
                    else:  # frame with no skeleton
                        clip_skeleton_3d.append(np.zeros((43, 3), dtype = float32))
